...
int comb_twolev_config[9] = { 8, 			/* a_l1size */
		256, 		/* a_l2size */
		8,			/* a_hist */
		FALSE, 	/* a_xor */
		8, 			/* b_l1size */
...
void sim_reg_options(struct opt_odb_t *odb)
{
	...
	*/
	...
	//---------------------------------------------------------------------------------------------------------
	// @587: Process the command line args for the branch predictor
	opt_reg_int_list(	odb, 									// option database
			"-bpred:2lev_comb",
			"Combined 2Level Predictor: <A_L1_size> <A_hist_size> <A_L2_size> <B_L1_size> <B_hist_size> <B_L2_size>",
			comb_config,					// pointer to option array
			comb_nelt, 						// total entries in option array
			&comb_nelt,						// number of entries parsed
			comb_config,					// default config for option array
			TRUE, 								// print during -dumpconfig?
			NULL, 								// optional user print format
			FALSE);								// accrue list across uses
	//---------------------------------------------------------------------------------------------------------
		...
		int argc, char **argv)        /* command line arguments */
{
	...
	// @587: Print message to verify we got to check options stage
	info("587: Begin out-of-order check options...");
	...
	//-----------------------------------------------------------------------------------------
	//	@587: For out-of-order execution, we need have it select our 2-level branch predictor
	else if (!mystricmp(pred_type, "2lev_comb"))
	{
		// @587: visual check to see that the 2-level combination predictor is chosen
		// Confirmed we do get here during out of order execution
		info("587: 2lev_comb predictor selected");
...
acess will support without overflowing the IFETCH -> DISPATCH QUEUE */
 ...
 speculative TLB misses */
	...
	//-------------------------------------------------------------------------------------
	/*	@587:	Start of the out of order simulator
	 *
	 */
	void sim_main(void)
	{
