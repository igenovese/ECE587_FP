...
sim_reg_options(struct opt_odb_t *odb)
{
	...
	opt_reg_string(odb, "-bpred",
			"branch predictor type {nottaken|taken|bimod|2lev|comb|2lev_comb}",
...
sim_check_options(struct opt_odb_t *odb, int argc, char **argv)
{
	...
	else if (!mystricmp(pred_type, "comb"))
	{
		...
		// Create the global branch predictor structure
...
#include "machine.def"
		...
		if (MD_OP_FLAGS(op) & F_CTRL)
		{
			...
			// if the predictor structure is allocated and valid
			if (pred)
			{
				...
				pred_PC = bpred_lookup(pred,
						  ...
						  &update_rec,			/* stash an update ptr */
